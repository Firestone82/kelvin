    ...
    validate(prefix, data, sourceMap) {
        ...
        for (let position = 0; position < stringValue.length; position++) {
            ...
            if (isLetter && !this.hasFlag(ValueRuleFlags.ALPHABET)) {
                errorList.push(err(source, "Contains alphabetic characters, which are not allowed."));
                break;
            }
...
const rules = new DictRule({
    ...
    async: new DictRule({
        llm: new DictRule({
            ...
            prompt_name: [new ValueRule(ValueRuleFlags.NUMERIC), 'ID of the custom prompt stored in the system.']
        })
    })
});
